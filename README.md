...
| `BACKUPS_MAX_COUNT` | `72` | Maximum number of backups kept |
...
By default at most 72 backups will be kept. Once reaching this amount of backups, the oldest one(s) will be deleted. A different amount can be configured using `BACKUPS_MAX_COUNT`. Note that `BACKUPS_MAX_AGE` will always be respected: if backups get too old, they will be deleted even if `BACKUPS_MAX_COUNT` was not yet reached.
