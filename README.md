...
| `PRE_START_HOOK` |  | Command to be executed before a server start is performed. Current start is blocked until this command returns. |
| `POST_START_HOOK` |  | Command to be executed after a server start was performed. Future restarts and update checks are blocked until this command returns. |
